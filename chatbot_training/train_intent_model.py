...
def main():
    ...
    # general_knowledge/build_index.py (that file's size pushed the TS
    # compiler's type-checker out of memory; this one gets the same fix
    # pre-emptively since its training set keeps growing every retrain).
    ts_type_declaration = (
        "type LinearModelShape = {\n"
        "  classes: string[];\n"
        "  terms: string[];\n"
        "  idf: number[];\n"
        "  coef: number[][];\n"
        "  intercept: number[];\n"
        "};\n\n"
        "type ChatbotIntentModelShape = {\n"
        "  modelName: string;\n"
        "  algorithm: string;\n"
        "  generative: boolean;\n"
        "  trainingRows: number;\n"
        "  crossValidationAccuracy: number;\n"
        "  intentModel: LinearModelShape;\n"
        "  scopeModel: LinearModelShape;\n"
        "};\n\n"
    )
    ...
    ts = (
        ...
        + ts_type_declaration
        + "export const CHATBOT_INTENT_MODEL: ChatbotIntentModelShape = "
