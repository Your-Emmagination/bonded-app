...
export const getAiErrorMessage = (error: unknown): string => {
  ...
  if (message.includes("Missing EXPO_PUBLIC_AI_WORKER_URL")) {
    return "B.E.A. is not configured in this build yet. Rebuild the APK after applying the AI worker URL.";
  }

  if (message.includes("aborted") || message.includes(`${AI_REQUEST_TIMEOUT_MS}`)) {
    return "B.E.A. timed out. The worker may be reachable but too slow to answer right now.";
  }

  if (message.includes("Network request failed") || message.includes("Failed to fetch")) {
    return "B.E.A. could not reach the server. Check the APK's internet access and the AI worker URL.";
  }

  if (message.includes("empty reply")) {
    return "B.E.A. responded with an empty message. The worker is reachable, but the upstream model reply failed.";
  }
